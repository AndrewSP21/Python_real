# 3. Пользователь вводит месяц в виде целого числа от 1 до 12. Сообщить к какому времени года относится
# месяц (зима, весна, лето, осень). Напишите решения через list и через dict.


while True:
    m = input('Введите номер месяца: ')
    if m.isdigit():
        if int(m) >= 1:
            if int(m) <= 12:
                m = int(m)
                break
    print('Ошибка данных, необходимо ввести число от 1 до 12')

# С помощью списков
month = ['Зима','Весна','Лето','Осень']

if m == 12 or m == 1 or m == 2:
    print(month[0],type(month))
elif m == 3 or m == 4 or m == 5:
    print(month[1],type(month))
elif m == 6 or m == 7 or m == 8:
    print(month[2],type(month))
else:
    print(month[3],type(month))

# С помощью словаря
mo = {12: 'Зима', 1: 'Зима', 2: 'Зима', 3: 'Весна', 4: 'Весна', 5: 'Весна', 6: 'Лето', 7: 'Лето', 8: 'Лето',
      9: 'Осень', 10: 'Осень', 11: 'Осень'}
print(mo[m], type(mo))
